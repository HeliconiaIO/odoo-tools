...
class ResConfigSettings(models.TransientModel):
    ...
    send_wish_employee = fields.Boolean(string='Send wish to employee')
    emp_wish_template_id = fields.Many2one('mail.template', string='Employee Email Template')
    send_wish_manager = fields.Boolean(string='Send wish to manager')
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].set_param("employee.send_wish_employee",
                                                  'True' if self.send_wish_employee else 'False')
        self.env['ir.config_parameter'].set_param("employee.emp_wish_template_id",
                                                  self.emp_wish_template_id.id or False)
        self.env['ir.config_parameter'].set_param("employee.send_wish_manager",
                                                  'True' if self.send_wish_manager else 'False')
        self.env['ir.config_parameter'].set_param("employee.manager_wish_template_id",
                                                  self.manager_wish_template_id.id or False)
        self.env['ir.config_parameter'].set_param("employee.reminder_before_day", self.reminder_before_day or 0)
