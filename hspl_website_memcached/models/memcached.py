...
try:
    from pymemcache.client.base import PooledClient, Client, MemcacheClientError, MemcacheUnknownCommandError, \
        MemcacheIllegalInputError, MemcacheServerError, MemcacheUnknownError, MemcacheUnexpectedCloseError
    ...
    _logger.info('hspl_website_memcached requires pymemcache (pip install pymemcache) %s.' % e)
...
def MEMCACHED_CLIENT():
    ...
    if not MEMCACHED__CLIENT__:
        try:
            ...
            # ~ else:
            MEMCACHED__CLIENT__ = PooledClient(MEMCACHED_SERVER, serializer=serialize_pickle,
                                               deserializer=deserialize_pickle, no_delay=MEMCACHE_NODELAY,
                                               connect_timeout=MEMCACHE_CONNECT_TIMEOUT, timeout=MEMCACHE_TIMEOUT)
            ...
            x = MEMCACHED__CLIENT__.stats('items')
            items = slab_stats = {y.decode('utf-8'): x.get(y) for y in x.keys()}
            slab_limit = {k.split(':')[1]: v for k, v in items.items() if
                          k.split(':')[2] == 'number'}  # slab -> limit
            ...
            key_lists = [MEMCACHED__CLIENT__.stats('cachedump', slab, str(limit)) for slab, limit in
                         slab_limit.items()]  # List of lists
            keys = [key for sublist in key_lists for key in
                    sublist.keys()]  # [4092067750, 3699334878 ...]             flattended list
            for key in keys:
...
class website(models.Model):
    ...
    def _flush_all_records(self):
        mc_delete(get_keys(status_code='all'))
...
def get_keys(db=None, match_any=False, **kwargs):
    ...
    x = MEMCACHED_CLIENT().stats('items')
...
def get_flush_page(keys, title, url='', delete_url=''):
    def append_key(rows, key, p):
        rows.append((
            ...
            p.get('module', '').replace('odoo.addons.', '').split('.')[0],
            p.get('flush_type'),
            p.get('key_raw'),
    ...
    return request.render("hspl_website_memcached.memcached_page", {
        'title': title,
        ...
        def response_wrap(*args, **kw):
            ...
            if routing.get('key'):  # Function that returns a raw string for key making
                ...
                key_raw = routing['key'](kw).format(path=request.httprequest.path,
                                                    session='%s' % {k: v for k, v in request.session.items() if
                                                                    len(k) < 40},
                                                    device_type='%s' % request.session.get('device_type', 'md'),
                                                    # xs sm md lg
                                                    context='%s' % {k: v for k, v in request.env.context.items() if
                                                                    not k == 'uid'},
                                                    ...
                                                    is_user='1' if (
                                                            request.website and request.website.is_user()) else '0',
                                                    employee='1' if request.env.ref(
                                                        'base.group_user') in request.env.user.groups_id else '0',
                                                    publisher='1' if request.env.ref(
                                                        'base.group_website_publisher') in request.env.user.groups_id else '0',
                                                    designer='1' if request.env.ref(
                                                        'base.group_website_designer') in request.env.user.groups_id else '0',
                                                    ).encode('latin-1', 'replace')
                # ~ raise Warning(request.env['res.users'].browse(request.uid).group_ids)
                key = str(MEMCACHED_HASH(key_raw))
            else:
                key_raw = ('%s,%s,%s' % (request.env.cr.dbname, request.httprequest.path, request.env.context)).encode(
                    'latin-1', 'replace')  # Default key
                key = str(MEMCACHED_HASH(key_raw))
                # ~ _logger.warn('\n\ndefault key_raw: %s\nkey: %s\n' % (key_raw, key))
            ...
            try:
                ...
                return werkzeug.wrappers.Response(status=500, headers=[
                    ...
                    (
                        'Server',
                        'Odoo %s Memcached %s' % (common.exp_version().get('server_version'), MEMCACHED_VERSION)),
                ])
            ...
            if page_dict and not page_dict.get('db') == request.env.cr.dbname:
                _logger.warn('Database violation key=%s stored for=%s  env db=%s ' % (
                    key, page_dict.get('db'), request.env.cr.dbname))
                page_dict = None

            # Blacklist
            if page_dict and any(
                    [p if p in request.httprequest.path else '' for p in kw.get('blacklist', '').split(',')]):
                page_dict = None

            if 'cache_viewkey' in kw.keys():
                if page_dict:
                    res = mc_meta(key)
                    view_meta = '<h2>Metadata</h2><table>%s</table>' % ''.join(
                        ['<tr><td>%s</td><td>%s</td></tr>' % (k, v) for k, v in res['page_dict'].items() if
                         not k == 'page'])
            ...
            cache_age = int(request.env['ir.config_parameter'].sudo().get_param('memcached.expiry.seconds'))
            ...
            if not page_dict:
                ...
                if response.template:
                    ...
                    response_list = [x.decode('utf-8') for x in response.response]
                    page = ''.join(response_list).encode('utf-8')
                flush_type = routing[
                    'flush_type'](kw).lower().encode(
                    'ascii', 'replace').decode(
                    'utf-8').replace(' ', '-') if routing.get(
                    'flush_type', None) else ""
                page_dict = {
                    'max-age': max_age,
                    'cache-age': cache_age,
            ...
            response = http.Response(
                base64.b64decode(page_dict.get('page')))  # always create a new response (drop response from controller)
            ...
            if page_dict.get('headers'):
                for k, v in page_dict['headers'].items():
                    ...
                    response.headers[k] = v
            # ~ _logger.warn('\n\ndirty headers: %s\n' % response.headers)
            response.headers['Cache-Control'] = 'max-age=%s,s-maxage=%s,%s' % (max_age, s_maxage, ','.join(
                [keyword for keyword in
                 ['no-store', 'immutable', 'no-transform', 'no-cache', 'must-revalidate', 'proxy-revalidate'] if
                 routing.get(keyword.replace('-', '_'))] + [
                    routing.get('private', 'public')]))  # private: must not be stored by a shared cache.
            ...
            response.headers['Server'] = 'Odoo %s Memcached %s' % (
                common.exp_version().get('server_version'), MEMCACHED_VERSION)
